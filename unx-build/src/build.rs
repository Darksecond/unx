...
pub fn build() -> Result<()> {
  ...
  cargo("kernel")?;
  cargo("bootloader-efi")?;

  FatBuilder::new("disk.fat")
    .file("kernel/target/x86_64-unx/release/kernel", "kernel.elf")
    .file("bootloader-efi/target/x86_64-unknown-uefi/release/bootloader-efi.efi", "efi/boot/bootx64.efi")
    .build()?;
  ...
  pub fn build(&mut self) -> Result<()> {
    ...
    for file in &self.files {
      ...
      for component in path.parent().context("path incorrect")?.components() {
        dir = dir.create_dir(component.as_os_str().to_str().context("cannot convert to string")?)?;
      }
...
pub fn build_gpt(fat_path: &str, gpt_path: &str) -> Result<()> {
  ...
  let partition = disk.partitions().get(&partition_id).context("Cannot find partition")?;
